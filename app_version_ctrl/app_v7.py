...
def draw_move_ranges(surface: pg.Surface, origin: tuple[int, int] | None, tmap: TileMap, available_ap: int) -> None:
    """
    Obstacle-aware move ranges. Always draws blue (1 AP) if available_ap >= 1;
    draws yellow (2 AP) only if available_ap >= 2.
    The origin can be the unit.grid or an inspection selection.
    """
...
def draw_debug(
    ...
    """HUD with turn/phase, hovered/selected, unit AP, and path info."""
    ...
    lines = [
        ...
        "ENTER/E: end turn   L-Click: move (player phase & enough AP)   R-Click: select/clear inspect tile   B: toggle obstacle   R: refill AP   ESC: quit",
    ]
...
def main() -> int:
    ...
    try:
        ...
        pg.display.set_caption("XCOM Iso — Turns, AP, Selection & Obstacles")
        clock = pg.time.Clock()
        ...
        while running:
            ...
            for e in pg.event.get():
                if e.type == pg.QUIT:
                    ...
                    if e.button == 1 and tm.phase is Phase.PLAYER:  # Left click → move if possible
            ...
            # AP rings + path preview use selection if present, else unit grid
            origin_for_ranges = selected if selected is not None else unit.grid
            available_ap = unit.ap if tm.phase is Phase.PLAYER else 0
            draw_move_ranges(screen, origin_for_ranges, tmap, available_ap)
            hovered = draw_hover(screen, pg.mouse.get_pos())
            path_info = draw_path_preview(screen, origin_for_ranges, hovered, tmap) if tm.phase is Phase.PLAYER else None

            draw_unit(screen, unit)
            draw_selection(screen, selected)
            draw_debug(screen, font, hovered, unit, selected, path_info, tm)
